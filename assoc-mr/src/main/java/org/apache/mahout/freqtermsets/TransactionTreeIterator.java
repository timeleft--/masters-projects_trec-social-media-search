...
/**
 * Generates a List of transactions view of Transaction Tree by doing Depth First Traversal on the
 * tree
 * structure
 */
final class TransactionTreeIterator extends AbstractIterator<Pair<IntArrayList, Long>> {
  ...
  TransactionTreeIterator(TransactionTree transactionTree, boolean onlyClosed) {
...
//      }
    ...
    } while ((onlyClosed && transactionTree.childCount(childId) > 0) ||
        (!onlyClosed && (sum == transactionTree.count(childId))));
    ...
    Pair<IntArrayList, Long> returnable = new Pair<IntArrayList, Long>(data,
        transactionTree.count(childId) - sum);
