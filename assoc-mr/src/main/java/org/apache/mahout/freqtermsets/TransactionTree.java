...
import org.apache.mahout.freqtermsets.TransactionTreeIterator;
...
public final class TransactionTree implements Writable,
	...
	public int addPatternUnique(IntArrayList myList, long addCount) {
		...
		// If it is a subpattern we don't want to do anything with it
		if (lengthAlreadyThere <= patternNodes.length) {
			for (int idx = 0; idx < lengthAlreadyThere; ++idx) {
				// if (patternNodes[idx] == -1) {
				// break;
				// }
				// If the pattern is completely included in the tree, this
				// method
				// want to add the unique part of it,
				// which is the increase in its count.. otherwise, the whole
				// pattern
				// is unique, add all the count
				addCount(patternNodes[idx], (addCountMode ? addCount
						- count(patternNodes[idx]) : addCount));
			}
		}
		return ret;
	}
